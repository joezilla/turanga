...
| 2026-08-03 | Bug fix (runtime — "Invalid model name openai-compatible/…"): openai-compatible models were registered in LiteLLM under `<connection-name>/<id>` but the agent references them as `<kind>/<id>` = `openai-compatible/<id>` (ModelSelector groups by kind), so a run never resolved. Register now uses the `openai-compatible/<id>` prefix (still distinct from the `openai/*` wildcard); `litellm_params` keeps `openai/<id>` + base_url. Unit-tested (gateway registration). **An existing compatible connection must be Refreshed (or removed + re-added) to re-register under the corrected prefix.** control-api 124 (+2), build 6/6, lint clean. |
