...
## Deferred from: code review of Epic 9 (Chat, stories 9.1–9.4) (2026-08-05)
- **Concurrent sends to one conversation collide on `turnIndex`.** `turnIndex = prior.length` (validateChatTurn) is a read-then-write with no per-conversation lock and no unique `(conversation_id, turn_index)` index; two racing POSTs (two tabs / direct API) both get index N and fork the thread with divergent history. The single-user UI serializes sends (Send disabled while running), so low-probability now. Fix: a unique `(conversation_id, turn_index)` index (small migration → the second insert fails cleanly) or a per-conversation advisory lock — when concurrency/multi-tenant matters.
- **Delete-during-stream reap (server-side residual).** After the web guard (disable rename/delete while streaming), a *direct-API* DELETE during an in-flight turn still orphans the live run (the run keeps spending against its cost key + times out at 120s; its status/cost writes no-op on the deleted row). Full fix: the DELETE route reaps a conversation's in-flight runs via the orchestrator (needs a "kill by conversation/run" orchestrator surface — the `controllers` map has `onKill` but no public kill-by-conversation).
- **Re-attach to a running turn on load.** Navigating away from a streaming thread closes the EventSource; returning renders the still-`running` persisted run statically (no live indicator, no re-subscribe) — the reply appears only on a manual reload. Fix: on load, detect a `running` run and re-open its `GET /runs/:id/events` SSE. Rare for a single user.
- **`GET /conversations/:id/runs` is unpaginated.** A long thread ships every run's full transcript on every load + every post-turn `loadRuns()`. Model history is windowed (last 20 exchanges, 9.4) but the read payload is not. Fix: paginate/limit the thread read when threads grow large.
