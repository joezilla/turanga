...
import { CONTRACT_VERSION, type GuardModelResponse, type ToolCallRequest } from "@turanga/contracts";
...
describe("guard model proxy", () => {
  it("maps a model request to a LiteLLM call and returns the completion", async () => {
    ...
    const res = await guard.proxyModel("01ARZ3NDEKTSV4RRFFQ69G5FAV", { v: CONTRACT_VERSION, runId: "r", model: "openai/gpt-4o", messages: [{ role: "user", content: "hi" }] });
...
describe("guard per-run socket lifecycle", () => {
  ...
  it("register provisions a UDS in the run's own subdir that serves the model contract; teardown removes it", async () => {
    ...
    const body = JSON.stringify({ v: CONTRACT_VERSION, runId: RUN, model: "m", messages: [{ role: "user", content: "hi" }] });
...
const readReq = { v: CONTRACT_VERSION, runId: RUN, connectionId: "gmail", op: "read" as const, params: { maxResults: 5 } };
...
describe("guard skill-permission enforcement (4.4, permission-first)", () => {
  ...
  const op = (o: "read" | "label" | "send") => ({ v: CONTRACT_VERSION, runId: RUN, connectionId: "gmail", op: o });
...
describe("guard tool broker (Story 6.4)", () => {
  ...
  const toolReq = (over: Partial<ToolCallRequest> = {}): ToolCallRequest => ({ v: CONTRACT_VERSION, runId: RUN, toolId: "t1", operation: "get_time", arguments: {}, ...over });
