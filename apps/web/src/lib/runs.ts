...
// Mirrors @turanga/contracts CONTRACT_VERSION (currently 8). Kept local so the web has no server dep.
export type RunMessage =
  | { type: "turn"; v: 8; role: "user" | "agent"; text: string }
  | { type: "metrics"; v: 8; latencyMs: number; tokens: number; costMicros: number }
  | { type: "refusal"; v: 8; kind: "egress" | "permission"; detail: string }
  | { type: "tool"; v: 8; toolId: string; toolName: string; operation: string; outcome: "ok" | "error" | "refused"; latencyMs: number; detail?: string } // Story 6.5
  | { type: "recall"; v: 8; memoryIds: string[]; count: number } // Story 8.3 — orchestrator-authored recall event
  | { type: "done"; v: 8; status: "succeeded" | "failed" | "killed" };
