...
// Mirrors @turanga/contracts CONTRACT_VERSION (currently 7). Kept local so the web has no server dep.
export type RunMessage =
  | { type: "turn"; v: 7; role: "user" | "agent"; text: string }
  | { type: "metrics"; v: 7; latencyMs: number; tokens: number; costMicros: number }
  | { type: "refusal"; v: 7; kind: "egress" | "permission"; detail: string }
  | { type: "tool"; v: 7; toolId: string; toolName: string; operation: string; outcome: "ok" | "error" | "refused"; latencyMs: number; detail?: string } // Story 6.5
  | { type: "recall"; v: 7; memoryIds: string[]; count: number } // Story 8.3 — orchestrator-authored recall event
  | { type: "done"; v: 7; status: "succeeded" | "failed" | "killed" };
